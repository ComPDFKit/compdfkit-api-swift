...
class PDFToCSV: NSObject {
    ...
    class func entrance() {
        ...
        self.client.createTask(url: CPDFConversion.PDF_TO_CSV) { taskModel in
            ...
            self.client.uploadFile(filepath: path!, password: "", params: [CPDFFileUploadParameterKey.isCsvMerge.string() : "1"], taskId: taskId) { uploadFileModel in
...
//                            }
    ...
    class func asyncEntrance() {
        ...
        Task { @MainActor in
            // Create a task
            let taskModel = await client.createTask(url: CPDFConversion.PDF_TO_CSV)
            let taskId = taskModel?.taskId ?? ""

            // upload File
            let path = Bundle.main.path(forResource: "test", ofType: "pdf")
            let uploadFileModel = await client.uploadFile(filepath: path ?? "",  password: "",params: [CPDFFileUploadParameterKey.isCsvMerge.string() : "1"], taskId: taskId)
