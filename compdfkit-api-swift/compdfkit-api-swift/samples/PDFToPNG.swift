...
class PDFToPNG: NSObject {
    ...
    class func entrance() {
        ...
        self.client.createTask(url: CPDFConversion.PDF_TO_PNG) { taskModel in
            ...
            self.client.uploadFile(filepath: path!, password: "", params: [:], taskId: taskId) { uploadFileModel in
...
//                            }
    ...
    class func asyncEntrance() {
        ...
        Task { @MainActor in
            // Create a task
            let taskModel = await client.createTask(url: CPDFConversion.PDF_TO_PNG)
