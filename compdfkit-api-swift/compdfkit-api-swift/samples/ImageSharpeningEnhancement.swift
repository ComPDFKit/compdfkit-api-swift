//
//  ImageSharpeningEnhancement.swift
//  compdfkit-api-swift
//
...
class ImageSharpeningEnhancement: NSObject {
    private static var client: CPDFClient = CPDFClient(publicKey: public_key, secretKey: secret_key)
    
    class func entrance() {
        // Create a task
        self.client.createTask(url: CPDFDocumentAI.MAGICCOLOR) { taskModel in
...
//                            }
    ...
    class func asyncEntrance() {
        ...
        Task { @MainActor in
            // Create a task
            let taskModel = await client.createTask(url: CPDFDocumentAI.MAGICCOLOR)
