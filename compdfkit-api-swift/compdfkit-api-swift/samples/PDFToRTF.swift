...
class PDFToRTF: NSObject {
    ...
    class func entrance() {
        ...
        self.client.createTask(url: CPDFConversion.PDF_TO_RTF) { taskModel in
            ...
            self.client.uploadFile(filepath: path!, password: "", params: [CPDFFileUploadParameterKey.isContainAnnot.string() : "1", CPDFFileUploadParameterKey.isContainImg.string() : "1"], taskId: taskId) { uploadFileModel in
...
//                            }
    ...
    class func asyncEntrance() {
        ...
        Task { @MainActor in
            // Create a task
            let taskModel = await client.createTask(url: CPDFConversion.PDF_TO_RTF)
